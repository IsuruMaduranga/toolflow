...
def get_structured_output_tool(pydantic_model: Any, streaming: bool = False) -> Callable[..., str]:
    ...
    structured_response_tool.__doc__ = f"""
        ...
        {"" if not streaming else "✓ You can call this tool multiple times as you build response"}
        {"" if streaming else "✓ You can call this tool only once with complete response"}
        {"" if streaming else "✓ This tool must be your last tool call in the conversation"}
        BEHAVIOR:
